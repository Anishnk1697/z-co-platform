    ...
    <div className="min-h-screen bg-white text-ink">
      <section className="pt-36 pb-20 lg:pt-52 lg:pb-32 px-4 sm:px-6 lg:px-8 max-w-5xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
          <span className="inline-flex items-center gap-2 px-3 py-1  bg-off border border-[rgba(0,0,0,0.08)] text-[10px] font-normal text-mid uppercase tracking-[0.18em] mb-6">Real Estate Development Company</span>
          <h1 className="text-4xl sm:text-5xl lg:text-6xl font-serif font-light leading-[1.06] mb-5">
            Real Estate Development<br /><span className="text-mid">Company — USA</span>
          </h1>
          <p className="text-xl text-mid font-light leading-relaxed max-w-3xl">Z-Co Development Corp. is a full-service real estate development company headquartered in Houston, Texas. We specialize in <Link to="/build-to-rent-developer-usa" className="underline hover:text-ink transition-colors">build-to-rent communities</Link>, <Link to="/healthcare-real-estate-developer" className="underline hover:text-ink transition-colors">healthcare real estate</Link>, <Link to="/mixed-use-real-estate-developer" className="underline hover:text-ink transition-colors">mixed-use developments</Link>, and <Link to="/eb5-real-estate-projects-usa" className="underline hover:text-ink transition-colors">EB-5 investment opportunities</Link> across the United States Sun Belt.</p>
        </motion.div>
      </section>

      <section className="py-20 border-t border-[rgba(0,0,0,0.07)] px-4 sm:px-6 lg:px-8">
        <div className="max-w-5xl mx-auto">
          <h2 className="text-3xl sm:text-4xl font-serif font-light text-ink mb-5">About Z-Co Development Corp.</h2>
          <div className="grid lg:grid-cols-2 gap-12">
            ...
            <div className="space-y-5 text-mid leading-relaxed">
              ...
              <p>Z-Co is actively seeking investors, landowners, <Link to="/real-estate-joint-venture-partnerships" className="underline hover:text-ink transition-colors">joint venture partners</Link>, and EB-5 capital partners to expand our platform across these high-conviction markets. We are committed to transparent communication, aligned incentive structures, and the highest standards of professional conduct in every relationship we build.</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-20 border-t border-[rgba(0,0,0,0.07)] bg-off px-4 sm:px-6 lg:px-8">
        <div className="max-w-5xl mx-auto">
          <h2 className="text-3xl sm:text-4xl font-serif font-light text-ink mb-4">What Sets Z-Co Apart</h2>
          <p className="text-mid mb-12 text-lg">Z-Co's integrated platform and replication model deliver advantages that traditional developers cannot match.</p>
      ...
      <section className="py-20 border-t border-[rgba(0,0,0,0.07)] px-4 sm:px-6 lg:px-8">
        <div className="max-w-5xl mx-auto">
          <h2 className="text-3xl sm:text-4xl font-serif font-light text-ink mb-5">Our Development Sectors</h2>
          <div className="grid md:grid-cols-2 gap-8">
            {[
              ...
              <div key={i} className="p-8  bg-white border border-[rgba(0,0,0,0.08)]">
                <h3 className="text-xl font-serif font-light text-ink mb-3">{s.title}</h3>
                <p className="text-mid text-sm leading-relaxed mb-5">{s.desc}</p>
                <Link to={s.link} className="inline-flex items-center gap-2 text-xs font-serif font-light text-ink uppercase tracking-widest hover:text-ink transition-colors">Learn More <ArrowRight className="w-3.5 h-3.5" /></Link>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-24 border-t border-[rgba(0,0,0,0.07)] px-4 sm:px-6 lg:px-8">
        <div className="max-w-3xl mx-auto text-center space-y-6">
          <h2 className="text-3xl sm:text-4xl font-serif font-light text-ink">Work with Z-Co Development</h2>
          <p className="text-mid text-lg leading-relaxed">Whether you're an investor seeking institutional-quality real estate returns, a landowner with a site in a high-growth market, or a capital partner looking for an experienced development operator — Z-Co is ready to have the conversation.</p>
          <div className="flex flex-wrap justify-center gap-4 pt-4">
            ...
            <Link to="/about" className="inline-flex items-center gap-2 px-8 py-4 border border-[rgba(0,0,0,0.1)] text-ink  font-normal uppercase text-[11px] tracking-[0.14em] hover:border-white transition-all">About Z-Co <ArrowRight className="w-4 h-4" /></Link>
          </div>
        </div>
      </section>
    </div>
