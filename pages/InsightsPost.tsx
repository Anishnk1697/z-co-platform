...
const renderSection = (section: BlogSection, i: number) => {
  switch (section.type) {
    case 'h2':
      return <h2 key={i} className="font-serif font-light text-2xl sm:text-3xl text-ink mt-12 mb-4 leading-tight">{section.content}</h2>;
    case 'h3':
      return <h3 key={i} className="font-serif font-light text-xl text-ink mt-8 mb-3">{section.content}</h3>;
    case 'p':
          ...
          <ul className="space-y-3">
            {section.items?.map((item, j) => (
              <li key={j} className="flex items-start gap-3 text-mid text-sm leading-relaxed">
                <span className="mt-1.5 w-1.5 h-1.5 bg-gold shrink-0" />
                {item}
              </li>
            ))}
          </ul>
  ...
  return (
    <div className="min-h-screen bg-gold text-white">
      ...
      <div className="pt-32 lg:pt-44 pb-0 px-4 sm:px-6 lg:px-8 max-w-3xl mx-auto">
        <Link to="/insights" className="inline-flex items-center gap-2 text-[11px] font-normal text-mid hover:text-ink uppercase tracking-[0.14em] transition-colors mb-10">
      ...
      <article className="px-4 sm:px-6 lg:px-8 max-w-3xl mx-auto">
        ...
        >
          <div className="flex flex-wrap items-center gap-3 mb-6">
            <span className={`inline-flex items-center gap-1.5 px-3 py-1 border text-[10px] font-normal uppercase tracking-[0.14em] ${categoryColors[post.category] || 'text-mid bg-off border-[rgba(0,0,0,0.08)]'}`}>
          ...
          <h1 className="font-serif font-light text-3xl sm:text-4xl lg:text-5xl text-ink leading-[1.1] mb-6">
            {post.title}
          </h1>
          ...
          <div className="mt-10  overflow-hidden bg-off border border-[rgba(0,0,0,0.08)] aspect-[16/7] flex items-center justify-center">
            ...
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
              <div className="text-center">
                <div className="text-[10px] font-normal text-gold uppercase tracking-[0.14em]">{post.category}</div>
                <div className="text-mid text-sm mt-1 font-light">Z-Co Development</div>
              </div>
            </div>
          </div>
        </motion.header>
        ...
        <div className="py-12 border-b border-[rgba(0,0,0,0.07)]">
          <div className="flex items-start gap-4 p-6  bg-off/60 border border-[rgba(0,0,0,0.08)]">
            <div className="w-10 h-10 bg-ink2 flex items-center justify-center shrink-0 text-[10px] font-normal text-white/60 uppercase tracking-widest">ZC</div>
            <div>
              <p className="font-serif font-normal text-sm text-ink mb-1">Z-Co Development</p>
              ...
              <div className="flex flex-wrap gap-3 mt-4">
                <Link to="/about" className="text-[10px] font-normal text-gold uppercase tracking-[0.14em] hover:text-ink transition-colors">About Z-Co →</Link>
                <Link to="/#invest" className="text-[10px] font-normal text-gold uppercase tracking-[0.14em] hover:text-ink transition-colors">Investor Access →</Link>
                <Link to="/portfolio" className="text-[10px] font-normal text-gold uppercase tracking-[0.14em] hover:text-ink transition-colors">Our Projects →</Link>
              </div>
            </div>
          </div>
        </div>
        ...
        {related.length > 0 && (
          <div className="py-12 pb-24">
            <p className="text-[10px] font-normal text-gold uppercase tracking-[0.28em] mb-6">Related Articles</p>
            <div className="grid sm:grid-cols-3 gap-5">
              {related.map(rel => (
                <Link key={rel.slug} to={`/insights/${rel.slug}`} className="group block p-5 border border-[rgba(0,0,0,0.08)] hover:border-gold/30 bg-off transition-all">
                  <span className={`inline-block px-2 py-0.5 border text-[8px] font-normal uppercase tracking-[0.14em] mb-3 ${categoryColors[rel.category] || 'text-mid bg-off border-[rgba(0,0,0,0.08)]'}`}>
                    {rel.category}
                  </span>
                  <h4 className="font-serif font-normal text-sm text-ink leading-snug group-hover:text-gold transition-colors mb-2">{rel.title}</h4>
